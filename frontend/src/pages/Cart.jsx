...
const Cart = () => {
                            ...
                            <div className='flex items-start gap-6'>
                                ...
                                <div>
                                    ...
                                    <div className='flex items-center gap-5 mt-2'>
                                        <p>{currency}{productData.price.toLocaleString('vi-VN')}</p>
                                        <p className='px-2 sm:px-3 sm:py-1 border bg-slate-50'>{item.size}</p>
                                    </div>
                                </div>
                            </div>
                            <input onChange={(e)=> e.target.value === '' || e.target.value === '0' ? null :updateQuantity(item._id,item.size,Number(e.target.value))} className='border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1 ' type="number" min={1} defaultValue={item.quantity}  />
                            <img onClick={()=> updateQuantity(item._id,item.size,0)} className='w-4 sm:w-5 mr-4 sm:py-1 cursor-pointer' src={assets.bin_icon} alt="" />
    ...
    ) : <p className='mt-5 border-t-2  font-medium text-gray-600 text-2xl flex align-center justify-center'>Giỏ hàng trống</p>
}
